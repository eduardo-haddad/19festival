...
//api vb artistas
$ch  = curl_init('http://acervo.videobrasil.org.br/api/artistas/artista/'.$id_artista1.'');
curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
$result = curl_exec($ch);
$artista1 = json_decode($result, true);
curl_close($ch);
$ch  = curl_init('http://acervo.videobrasil.org.br/api/artistas/artista/'.$id_artista2.'');
curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
$result = curl_exec($ch);
$artista2 = json_decode($result, true);
curl_close($ch);
$ch  = curl_init('http://acervo.videobrasil.org.br/api/artistas/artista/'.$id_artista3.'');
curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
$result = curl_exec($ch);
$artista3 = json_decode($result, true);
curl_close($ch);
...
$nome_artista = $idioma == 'pt' ? 'Marinos Koutsomichalis | Grécia + <br />Maria Varela | Grécia +<br />Afroditi Psarra | Grécia' : 'Marinos Koutsomichalis | Greece + Maria Varela | Greece<br />+ Afroditi Psarra | Greece';
$nome_artista1 = 'Marinos Koutsomichalis';
$nome_artista2 = 'Maria Varela';
$nome_artista3 = 'Afroditi Psarra';
    ...
    <script>
        ...
        $(document).ready(
            function(){
                ...
                paletaArtistas($('#artista-bio1'));
                paletaArtistas($('#artista-bio2'));
                paletaArtistas($('#artista-bio3'));
    ...
    <main class="container" id="container">
        ...
        <div style="height:36px; clear: both">&nbsp;</div>
            ...
            <div class="col-sm-12 col-md-6" style="float: right;">
                <div>
                    <span id="artista-bio1"><?php foreach($artista1["biografia"] as $bio=>$valor) {echo($idioma == $valor['idioma'] ? '<strong>' . $nome_artista1 . '</strong>' . $valor['biografia'] : null);}?></span>
            ...
            <div class="col-sm-12 col-md-6" style="float: right;">
                <div>
                    <span id="artista-bio2"><?php foreach($artista2["biografia"] as $bio=>$valor) {echo($idioma == $valor['idioma'] ? '<strong>' . $nome_artista2 . '</strong>' . $valor['biografia'] : null);}?></span>
            ...
            <div class="col-sm-12 col-md-6" style="float: right;">
                <div>
                    <span id="artista-bio3"><?php foreach($artista3["biografia"] as $bio=>$valor) {echo($idioma == $valor['idioma'] ? '<strong>' . $nome_artista3 . '</strong>' . $valor['biografia'] : null);}?></span>
